class PostSchedule {
  constructor({ client, config, httpHandler }) {
    ...
    this.db = client.db(config.dbName, config.dbConfig).collection("post_schedules");
  }
  ...
  async create(data) {
    try {
      ...
      return {
        ...
        stack: null,
        data: { schedule_id: result.insertedId.toString() },
      };
    } catch (error) {
  ...
  async deleteAll() {
    try {
      ...
      console.log("Error deleting all schedules :>>>>>>>>>>>>>>>", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
