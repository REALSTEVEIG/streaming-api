class RolePermission {
  constructor({ client, config, httpHandler }) {
    ...
    this.db = client.db(config.dbName, config.dbConfig).collection("role_permissions");
  }
  ...
  async create(data) {
    try {
      ...
      return {
        ...
        stack: null,
        data: { permission_id: result.insertedId.toString() },
      };
    } catch (error) {
      console.log("Error creating role permission :>>>>>>>>>>>>", error);
      return {
        success: false,
  ...
  async update({ id, data }) {
    try {
      ...
      console.log("Error updating role permission :>>>>>>>>>>>>>>", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
  ...
  async get({ id, values }) {
    try {
      ...
      console.log("Error getting role permission by id :>>>>>>>>>>>>", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
  ...
  async search({ query, search_field, size, page, limit, values }) {
    try {
      ...
      console.log("Error searching role permissions :>>>>>>>>>>", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
  ...
  async getAll({ query, size, page, limit, values }) {
    try {
      ...
      console.log("Error getting all role permissions :>>>>>>>>>>>>.", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
  ...
  async deleteAll() {
    try {
      ...
      console.log("Error deleting all role permissions :>>>>>>>>>>>>", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
