class KtnPressImages {
  constructor({ client, config, httpHandler }) {
    this.httpHandler = httpHandler;
    this.id = config.id;
    this.db = client.db(config.dbName, config.dbConfig).collection("ktn_post_images");
  }
  ...
  async get({ id, values }) {
    try {
      ...
      if (!result) {
        return {
          success: false,
          message: "Images records not found!",
          status: this.httpHandler.ERROR_400,
          stack: null,
          data: null,
        };
