class StreamCredential {
  constructor({ client, config, httpHandler }) {
    ...
    this.id = config.id;
    this.db = client.db(config.dbName, config.dbConfig).collection("stream_credentials");
  }
  ...
  async create(data) {
    try {
      ...
      return {
        ...
        stack: null,
        data: { credential_id: result.insertedId.toString() },
      };
    } catch (error) {
      return {
        success: false,
...
module.exports = StreamCredential;
