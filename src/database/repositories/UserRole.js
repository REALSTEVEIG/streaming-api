class UserRole {
  constructor({ client, config, httpHandler }) {
    ...
    this.db = client.db(config.dbName, config.dbConfig).collection("user_roles");
  }
  ...
  async create(data) {
    try {
      ...
      return {
        ...
        stack: null,
        data: { user_role_id: result.insertedId.toString() },
      };
    } catch (error) {
      console.log("Error creating user role :>>>>>>>>>>>>>>>", data, error);
      return {
        success: false,
  ...
  async update({ id, data }) {
    try {
      ...
      console.log("Error updating user role :>>>>>>>>>>>>>>>>", data, error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
  ...
  async search({ query, search_field, size, page, limit, values }) {
    try {
      ...
      console.log("Error searching user roles :>>>>>>>>>>>>>>", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
  ...
  async deleteAll() {
    try {
      ...
      console.log("Error deleting all user roles :>>>>>>>>>>>>>>>>>>>", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
