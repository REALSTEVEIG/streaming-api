class KtnPressReleases {
  constructor({ client, config, httpHandler }) {
    this.httpHandler = httpHandler;
    this.id = config.id;
    this.db = client.db(config.dbName, config.dbConfig).collection("ktn_posts");
  }
  ...
  async get({ id, values }) {
    try {
      ...
      console.log("Error getting posts repositories", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
  ...
  async getAggregated(data) {
    try {
      ...
      console.log("Error getting posts aggregated :>>>>>>>>>>>>>>>>>", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
  ...
  async getAllAggregated(query) {
    try {
      ...
      console.log("Error getting aggregated posts :>>>>>>>>>>>....", error);
      return {
        success: false,
        message: "Internal server error",
        status: this.httpHandler.ERROR_500,
...
module.exports = KtnPressReleases;
